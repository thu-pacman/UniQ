...
# command for no_mpi
MPIRUN_CONFIG="" ./check_wrapper.sh $name -DGPU_BACKEND=mix -DSHOW_SUMMARY=on -DSHOW_SCHEDULE=off -DUSE_DOUBLE=on -DEVALUATOR_PREPROCESS=on -DENABLE_OVERLAP=on -DUSE_MPI=off -DDISABLE_ASSERT=on -DMAT=7 2>&1 | tee $name/std.out
