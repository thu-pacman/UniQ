...
extern __shared__ cpx shm[1<<LOCAL_QUBIT_SIZE];
...
__device__ __forceinline__ void U1Hi(int hiIdx, cpx p) {
    cpx hi = shm[hiIdx];
    shm[hiIdx] = make_cuComplex(COMPLEX_MULTIPLY_REAL(hi, p), COMPLEX_MULTIPLY_IMAG(hi, p));
}

__device__ __forceinline__ void USingle(int loIdx, int hiIdx, cpx v00, cpx v01, cpx v10, cpx v11) {
    cpx lo = shm[loIdx];
    cpx hi = shm[hiIdx];
...
__device__ __forceinline__ void GCCSingle(int loIdx, int hiIdx, cpx p) {
    cpx lo = shm[loIdx];
...
__device__ __forceinline__ void GCC(int idx, cpx p) {
    cpx v = shm[idx];
    ...
    for (int i = 0; i < numGates; i++) {
        ...
        if (!controlIsGlobal) {
            ...
            if (!targetIsGlobal) {
                ...
                int lo = ((threadIdx.x >> targetQubit) << (targetQubit + 1)) | (threadIdx.x & maskTarget);
                int hi = lo | (1 << targetQubit); 
                switch (deviceGates[i].type) {
                    FOLLOW_NEXT(GOC)
                    FOLLOW_NEXT(CU1)
...
__device__ void fetchData(cpx* a, unsigned int* threadBias, unsigned int idx, unsigned int blockHot, unsigned int enumerate, int numLocalQubits) {
    if (threadIdx.x == 0) {
        int bid = blockIdx.x;
        unsigned int bias = 0;
...
__device__ void saveData(cpx* a, unsigned int* threadBias, unsigned int enumerate) {
    unsigned int bias = blockBias | threadBias[threadIdx.x];
    int x;
    unsigned y;
    for (x = ((1 << (LOCAL_QUBIT_SIZE - THREAD_DEP)) - 1) << THREAD_DEP | threadIdx.x, y = enumerate;
...
template <unsigned int blockSize>
__global__ void run(cpx* a, unsigned int* threadBias, int* loArr, int* shiftAt, int numLocalQubits, int numGates, unsigned int blockHot, unsigned int enumerate) {
